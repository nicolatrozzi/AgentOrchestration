...
from src.deploy import DeploymentStatus, deploy_from_manifest
...
def cli():
    ...
    if args.command == "init":
        ...
        result = deploy_from_manifest(args.manifest)
        if result.status == DeploymentStatus.BLOCKED:
            print(f"Rollout blocked: {result.error}", file=sys.stderr)
            sys.exit(2)
        print(
            "Deployment ready: "
            f"{result.active_version} serving after "
            f"{len(result.migration_results)} migration(s)"
        )
