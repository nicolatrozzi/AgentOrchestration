...
def cli():
    ...
    flags_parser = subparsers.add_parser(
        "validate-flags",
        help="Validate rendered production feature flags",
    )
    flags_parser.add_argument(
        "rendered",
        help="Path to rendered per-service feature flags",
    )
    flags_parser.add_argument(
        "--manifest",
        help="Path to required feature flag manifest",
    )
    ...
    if args.command == "init":
        ...
        except (OSError, FeatureFlagValidationError) as exc:
        ...
        try:
            manifest = (
                load_feature_flag_manifest(args.manifest)
                if args.manifest
                else default_feature_flag_manifest()
            )
